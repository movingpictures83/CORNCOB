...
run <- function() {
   ...
   da_analysis <<- contrastsTest(formula = ~ Description,
phi.formula = ~ Description,
contrasts_DA = list("B2 - B1","B3 - B1","B3 - B2"),
data = physeq,
fdr_cutoff = 0.05)
...
output <- function(outputfile) {
  ...
  write.csv(da_analysis$discriminant_taxa_DA, paste(outputfile,"DA","csv", sep="."))
